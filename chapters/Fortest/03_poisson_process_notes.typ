...
// 对应图片中关于商场营业时间的题目（主要部分）
#kbox("题目描述：非齐次泊松过程（商场模型）")[
  *场景*：某商店每天8时开始营业。从8时到11时顾客到来的速率线性增加。
  - 8时顾客流量为 5人/小时 ($lambda(8)=5$)。
  - 11时顾客流量达到最高峰 20人/小时 ($lambda(11)=20$)。
  - 假设在不重叠的时间间隔内，到达商店的顾客数相互独立（非齐次泊松过程）。

  *问题*：
  1. 求 8:30 ~ 9:30 这段时间内到达商店顾客数的分布及期望。
  2. 求这段时间内没有顾客到达的概率 $P(N=0)$。
]
...
已知 $lambda(t)$ 在 $t in [8, 11]$ 上呈线性增长。设 $lambda(t) = k t + b$。
由边界条件：
$ cases(lambda(8) = 8k + b = 5, lambda(11) = 11k + b = 20) $

解得 $k = 5, b = -35$。
故强度函数为（对应笔记右侧红笔修正）：
$ lambda(t) = 5t - 35, quad 8 <= t <= 11 $

*2. 计算期望值 (8:30 - 9:30)*

非齐次泊松过程在时间段 $(t_1, t_2]$ 内的计数 $N(t_2) - N(t_1)$ 服从参数为 $integral_{t_1}^{t_2} lambda(t) dif t$ 的泊松分布。

目标时间段：8:30 ($t=8.5$) 到 9:30 ($t=9.5$)。

$
E[N(9.5) - N(8.5)] &= integral_{8.5}^{9.5} lambda(t) dif t \
&= integral_{8.5}^{9.5} (5t - 35) dif t \
&= [2.5t^2 - 35t]_{8.5}^{9.5} \
& "（利用平方差公式简化计算）" \
&= 2.5(9.5^2 - 8.5^2) - 35(9.5 - 8.5) \
&= 2.5(9.5 - 8.5)(9.5 + 8.5) - 35 times 1 \
&= 2.5 times 1 times 18 - 35 \
&= 45 - 35 \
&= 10
$

故该时段内到达人数 $N tilde P(10)$。
- 期望 $E[N] = 10$
- 方差 $D[N] = 10$ (对应笔记左下角红笔修正 $D X = integral lambda dif t = 10$)

*3. 计算概率*

求没有顾客到达的概率 ($k=0$)：
$
P(N=0) = (10^0)/(0!) e^(-10) = e^(-10)
$

// 对应图片左下角关于 EX, EY, DX 的片段
#intuition("知识点延伸：复合泊松过程 (手写笔记左下角)")[
  *题目片段*：
  设 $X(t)$ 为复合泊松过程，表示总人数或总效应。
  - 到达过程是参数为 $lambda$ 的泊松过程。
  - 每次到达产生的数量 $Y_i$ 独立同分布。
  - 笔记示例分布：$P(Y=4) = P(Y=1) = 1/2$。
  
  *核心公式*：
  若 $X(t) = sum_{i=1}^{N(t)} Y_i$，则：
  1. *期望*：$E[X(t)] = lambda t E[Y]$
  2. *方差*：$D[X(t)] = lambda t E[Y^2]$

  *本题数据计算*：
  - $E[Y] = 1 dot 0.5 + 4 dot 0.5 = 2.5$
  - $E[Y^2] = 1^2 dot 0.5 + 4^2 dot 0.5 = 0.5 + 8 = 8.5$
  
  假设 $lambda t = 2.5$ (根据笔记 $E X = 2.5 dot 2.5$ 推断)，则：
  - $E[X] = 2.5 times 2.5 = 6.25$
  - $D[X] = 2.5 times 8.5 = 21.25$
]